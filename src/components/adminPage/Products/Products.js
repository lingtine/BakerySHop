                    ...
                    <div className="admin-table__info">
                        <table className="admin-table__info--show">
                            ...
                            <tbody className="admin-table__info--data">
                                {loading ? <div></div> : displayedProducts.map((product,index) => (
                                    <tr style={{textAlign: 'center'}} key={product.id}>
                                        <td>{product.id}</td>
                                        <td style={{width: '100px'}}><img className="img-product" src={`data:image/png;base64,${product.image}`}/></td>
                                        <td>{product.name}</td>
                                        <td>{product.unit_price}</td>
                                        <td>{product.stock}</td>
                                        <td>{product.unit}</td>
                                        <td>
                                        <Link onClick={() => handleEditProduct(product)} to={`/admin/editproduct/${product.id}`}><FiEdit/></Link>
                                         <AiOutlineDelete onClick={() => alertDelete(product.name, product.id)}/></td>
                                    </tr>
                                ))}
